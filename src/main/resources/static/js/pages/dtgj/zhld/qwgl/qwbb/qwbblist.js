...
layui.config({
    ...
    function loadData() {
        ...
        //监听工具条
        table.on('tool(qwbb)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
    ...
    function searchTable() {
        mCurr = 1;
        mSelData.xm = $('#xm').val(), mSelData.fbsjBegin = $('#fbsjBegin').val(), mSelData.fbsjEnd = $('#fbsjEnd').val(), mSelData.wczt = $('#wczt').val()
        reloadTableAll();
    }
    //表格刷新
