...
layui.config({
    ...
    function showEdit(varType, varId) {
        var tmpTitle = '详情信息';
        var tmpUrl = '/dtgj/zhld/yjct/edit?type=' + varType + '&menuid=' + $.getUrlParam("id") + '&id=' + varId;
    ...
    function delfiles(varIds) {
        ...
        layui.$.ajax({
            type: 'post',
            url: mFjxxUrlTop + '/deletebypid',
            data: { ids: varIds, urltop: 'DTGJ/YJCT/' },
            dataType: 'json',
            headers: { token: localStorage["token"] },
    ...
    //绑定按钮事件
    $('#addyjct').on('click', function () {
        showEdit('add', '');
    });
    $('#deleetyjct').on('click', deleteItemAll);
    $('#selyjct').on('click', searchTable);
