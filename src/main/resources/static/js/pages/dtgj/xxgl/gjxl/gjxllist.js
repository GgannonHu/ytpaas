...
layui.config({
    ...
    function loadDataData() {
        table.render({
            ...
            cols: [[
                ...
                { field: 'GJXLZDZ', title: '终点站', width: '12%' },
        ...
        function cloCZ(d) {
            ...
            tmpRet += '<a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="selzd" title="公交站点信息" ><i class="fa fa-bus"></i></a>';
            return tmpRet;
        }
        ...
        table.on('tool(list)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent == 'del') { //删除
                ...
                var tmpTitle = '站点信息';
                var tmpUrl = '/dtgj/xxgl/gjzd/list?id=listGjzd&xlbm=' + data.GJXLBM + '&zdmc=' + mSelData.zdmc;
                //从桌面打开
    ...
    function showEdit(varType, varId) {
        var tmpTitle = '详情信息';
        var tmpUrl = '/dtgj/xxgl/gjxl/edit?type=' + varType + '&menuid=' + $.getUrlParam("id") + '&id=' + varId;
    ...
    function setIsDelByBm(varId, varBm) {
        ...
        $.ajax({
            type: 'get',
            url: '/api/dtgj/xxgl/gjxl/getisdelbybm',
            headers: { token: localStorage["token"] },
            data: { bm: varBm },
            dataType: 'json',
            success: function (data) {
