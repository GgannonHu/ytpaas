...
; !function () {
    ...
    layui.define(['jquery', 'layer', 'element', 'form'], function (exports) {
        ...
        //构造函数
        var Winui = function () {
        	var mySettings = $.parseJSON($.cookie("settings"));
            this.v = '1.0.0';  //版本号
            this.path = getPath;
            this.settings = {
                color: mySettings.color,
                taskbarMode: 'bottom',
                bgSrc: mySettings.bgSrc,
                lockBgSrc: mySettings.lockBgSrc,
...
//                    }
            ...
            , resetColor: function (color) {
                if (color) {
                    ...
                    $.post("/Main/setColor?color="+color);
                    //缓存数据
            ...
            , resetBg: function (bgSrc) {
                if (bgSrc) {
                    ...
                    bgSrc = bgSrc.substring(bgSrc.indexOf("/images"));
                    $.post("/Main/setBgSrc?bgSrc="+bgSrc);
                    //缓存数据
//                    layui.data('winui', { key: 'settings', value: this.settings });
                }
                this.renderBg();
            ...
            , resetLockBg: function (bgSrc) {
                if (bgSrc) {
                    ...
                    bgSrc = bgSrc.substring(bgSrc.indexOf("/images"));
                    $.post("/Main/setLockBgSrc?bgSrc="+bgSrc);
                    //缓存数据
                    //layui.data('winui', { key: 'settings', value: this.settings });
                }
            }
